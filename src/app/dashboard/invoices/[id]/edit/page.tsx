...
import { formatCurrency, UNITS, generateId } from "@/lib/utils";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        console.error("Invoice update failed:", resp.status, err);
        return;
      }
  ...
  if (loading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="mt-4 text-gray-500 text-sm">Loading invoice...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Update invoice details</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input type="date" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
          </div>
        </div>
      </div>
