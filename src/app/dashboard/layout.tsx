...
function DashboardInner({ children }: { children: ReactNode }) {
  ...
  const { companies, activeCompanyId, setActiveCompanyId, isLoading: companiesLoading } = useCompany();

  useEffect(() => {
    const startedAt = Date.now();
    fetch("/api/auth/session")
      .then((r) => {
        console.log(`[dashboard-layout] /api/auth/session responded in ${Date.now() - startedAt}ms with status ${r.status}`);
        return r.json();
      })
      .then((data) => {
      ...
      .catch((err) => {
        console.error(`[dashboard-layout] /api/auth/session failed after ${Date.now() - startedAt}ms:`, err);
        router.push("/");
      });
  }, [router]);
  ...
  if (!user || companiesLoading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="animate-spin rounded-full h-12 w-12 border-4 border-[#1e3a5f] border-t-transparent"></div></div>;
