  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/quotations/new" className="px-4 py-2 bg-[#1e3a5f] text-white rounded-lg text-sm font-medium hover:bg-[#152b48] transition flex items-center gap-2">
                ...
                {quotations.map((q: any) => (
                  <tr key={q.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setViewQuotation(q)}>
                    ...
                    <td className="px-6 py-4 text-right" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => setActivityLogModal({ id: q.id, number: q.quotationNumber })} className="text-indigo-600 hover:text-indigo-800 text-sm mr-2 font-medium" title="View edit history">📋 History</button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setViewQuotation(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <ActivityLog documentType="quotation" documentId={viewQuotation.id} />
            </div>
          </div>
        </div>
      )}
